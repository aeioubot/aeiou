...
module.exports = class WaterCommand extends Command {
	...
	async run(msg, {personToWater}) {
		if (!personToWater) {
			...
			if (userPlantData.activeSeed.watered && userPlantData.inventory.includes('WaterTray')) {
				userPlantData.activeSeed.lastEvent = `You overwatered your plant, but luckily you have a water tray.`;
				if (Math.floor(Math.random() * 15) === 0) userPlantData.inventory.splice(userPlantData.inventory.indexOf('WaterTray'), 1);
				return plants.storePlant(userPlant).then(() => msg.say(`You overwatered your plant, but fortunately you have a water tray. :shower:`));
			}
		...
		if (notMineData.activeSeed.watered && notMineData.inventory.includes('WaterTray')) {
			notMineData.activeSeed.lastEvent = `${msg.member.displayName} overwatered your plant, but luckily you have a water tray.`;
			if (Math.floor(Math.random() * 12)) notMineData.inventory.splice(notMineData.inventory.indexOf('WaterTray'), 1);
			return plants.storePlant(notMine).then(() => msg.say(`Shame on you. You overwatered ${personToWater.displayName}'s plant, but they have a water tray. :shower:`));
		}

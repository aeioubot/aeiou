...
module.exports = class ReplyCommand extends Command {
	...
	async run(msg, args) {
		...
		let testIfCustomReactionExists = provider.get(msg.guild, 'customReactions', []).find((x) => {
		...
		case 'add':
			...
			let toBePushed = provider.get(msg.guild, 'customReactions', []);
			toBePushed.push({
				trigger: trigger,
				content: content,
			});
		...
		case 'del':
			...
			let toBePushedDelete = provider.get(msg.guild, 'customReactions', []); // gets all the custom reactions for this guild
		...
		case 'list': // Lists the triggers in the guild.
			let triggerArrayOfObjects = provider.get(msg.guild, 'customReactions', []); // These 3 lines grab the all the triggers in this guild, and put them in an array.
			let triggerArray = [];

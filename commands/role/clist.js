...
module.exports = class CListCommand extends Command {
	...
	async run(msg) {
		...
		donors.forEach((donor) => message += `${msg.guild.members.find('id', donor.id) ? msg.guild.members.find('id', donor.id).user.username.padEnd(35) : `ID: ${donor.id}`.padEnd(35)}${msg.guild.roles.find('id', donor.role).name}\n`);
		message += '```';
		msg.say(message);
	}
};

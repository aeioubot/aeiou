...
// Credit: https://github.com/WeebDev

/**
 * Uses a MySQL database to store settings with guilds
...
class SequelizeProvider extends SettingProvider {
	...
	/**
	 * @param {MySQLDatabase} db - Database for the provider
	 */
	constructor(db) {
		super();
	...
	async init(client) {
		...
		for (const row of rows) {
			...
			try {
				...
				client.emit('warn', `MySQLProvider couldn't parse the settings stored for guild ${row.dataValues.guild}.`);
				continue;
			}

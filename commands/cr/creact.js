	...
	async run(msg, { option, trigger, content }) {
		...
		switch (option.toLowerCase()) {
			case 'add': {
				...
				if (trigger === '' || content === '' || trigger.replace(/ /g, '').length === 0 || content.some((x) => x.replace(/ /g, '').length === 0)) return msg.say('The custom reaction content or trigger can\'t be empty.'); // Because of default arguments, detecting an empty trigger or content when adding is necessary.
				...
				return reactDB.addToCache({
					...
					trigger: trigger,
					content: content,
				}).then(() => msg.say(`Reaction added, **${trigger}** will cause me to say ${content.length === 1 ? '**' + content[0] + '**' : 'one of **' + content.join(', ') + '**'}.`));
			}
			case 'edit': {
				if (!msg.member.hasPermission('MANAGE_MESSAGES') && !this.client.isOwner(msg.author.id)) return msg.say('You need permission to manage messages in order to manage custom reacts.');
				if (trigger === '' || content === '' || trigger.replace(/ /g, '').length === 0 || content.some((x) => x.replace(/ /g, '').length === 0)) return msg.say('The custom reaction content or trigger can\'t be empty.'); // Because of default arguments, detecting an empty trigger or content when adding is necessary.
				...
				return reactDB.editInCache({
					...
					trigger: trigger,
					content: content,
				}).then(msg.say(`Reaction edited! I will now say ${content.length === 1 ? '**' + content[0] + '**' : 'one of **' + content.join(', ') + '**'} in response to **${trigger}**.`));
			}
